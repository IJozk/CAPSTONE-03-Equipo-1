    ...
    <div class="space-y-6">
      ...
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-gray-600 mt-1">Administra todos los talleres extraescolares del colegio</p>
        </div>
        <button
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Costo Adicional</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Estado</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Acciones</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-if="tallerStore.talleres.length === 0">
                <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                  No hay talleres registrados
                </td>
              </tr>
              <tr v-for="taller in tallerStore.talleres" :key="taller.taller_id" class="hover:bg-gray-50">
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  <div v-if="taller.Horarios && taller.Horarios.length > 0" class="space-y-1">
                    <div v-for="horario in taller.Horarios" :key="horario.horario_id" class="text-sm">
                      <span class="font-medium text-gray-900">{{ horario.dia_semana }}</span>
                      <span class="text-gray-600 ml-1">
                        {{ formatTime(horario.hora_inicio) }} - {{ formatTime(horario.hora_termino) }}
                      </span>
                    </div>
                  </div>
                  <span v-else class="text-sm text-gray-400">Sin horarios</span>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  <div class="flex items-center">
                    ...
                    {{ taller.inscritos || 0 }} / {{ taller.capacidad_maxima }}
                  </div>
                  <div class="mt-1">
                    <div class="w-20 bg-gray-200 rounded-full h-2">
                      <div
                        ...
                        :class="[
                          ((taller.inscritos || 0) / taller.capacidad_maxima) >= 0.9 
                            ? 'bg-red-500' 
                            : ((taller.inscritos || 0) / taller.capacidad_maxima) >= 0.7 
                            ? 'bg-yellow-500' 
                            : 'bg-green-500'
                        ]"
                        :style="{ width: `${Math.min(((taller.inscritos || 0) / taller.capacidad_maxima) * 100, 100)}%` }"
                      ></div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  {{ taller.costo_adicional ? `$${taller.costo_adicional.toLocaleString()}` : 'Gratis' }}
                </td>
          ...
          <div class="p-6">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-6">
              ...
              <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <option v-for="sala in salaStore.salas" :key="sala.sala_id" :value="sala.sala_id">
                        {{ sala.nombre }}
                      </option>
                    </select>
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  />
                  <p class="text-xs text-gray-500 mt-1">Dejar vacío si es gratuito</p>
                </div>
              </div>
              ...
              <div class="space-y-4 border-t pt-4">
                ...
                <div v-if="formData.horarios.length === 0" class="text-center py-8 bg-gray-50 rounded-lg">
                  <svg class="w-12 h-12 mx-auto text-gray-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <p class="text-gray-500 text-sm">No hay horarios agregados</p>
                </div>
                ...
                <div v-else class="space-y-3">
                  ...
                  >
                    <div class="flex items-start justify-between mb-3">
                      <span class="text-sm font-medium text-gray-700">Horario {{ index + 1 }}</span>
                      <button
                        type="button"
...
// import type { Taller, CreateTallerDTO, Horario } from '@/types/taller.types'

// Simulación de stores para la vista
const tallerStore = ref({
  loading: false,
  error: null,
  talleres: [
    {
      ...
      descripcion: 'Desarrollo de habilidades futbolísticas para niños de 6 a 12 años',
      sala_id: '1',
      Sala: { nombre: 'Cancha deportiva' },
      capacidad_maxima: 20,
      inscritos: 15,
    ...
    },
    {
      ...
      descripcion: 'Técnicas básicas de pintura y dibujo artístico',
      sala_id: '2',
      Sala: { nombre: 'Sala de arte' },
      capacidad_maxima: 15,
      inscritos: 15,
...
const salaStore = ref({
  salas: [
    { sala_id: '1', nombre: 'Cancha deportiva' },
    { sala_id: '2', nombre: 'Sala de arte' },
    { sala_id: '3', nombre: 'Laboratorio de computación' },
    { sala_id: '4', nombre: 'Biblioteca' }
  ]
})
...
const tallerToDelete = ref<any | null>(null)
const tallerToActivate = ref<any | null>(null)

// Datos del formulario
const formData = ref({
  nombre: '',
  descripcion: null as string | null,
  sala_id: null as string | null,
  capacidad_maxima: 0,
  costo_adicional: null as number | null,
  estado_activo: true,
  horarios: [] as Array<{
    dia_semana: string
    hora_inicio: string
...
const openEditModal = (taller: any) => {
  isEditing.value = true
  editingId.value = taller.taller_id
  formData.value = {
    nombre: taller.nombre,
    descripcion: taller.descripcion,
    sala_id: taller.sala_id,
    capacidad_maxima: taller.capacidad_maxima,
    costo_adicional: taller.costo_adicional,
    estado_activo: taller.estado_activo,
    horarios: taller.Horarios ? taller.Horarios.map((h: any) => ({
      dia_semana: h.dia_semana,
      hora_inicio: formatTime(h.hora_inicio),
      hora_termino: formatTime(h.hora_termino)
    })) : []
  }
  showModal.value = true
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    alert(isEditing.value ? 'Taller actualizado exitosamente' : 'Taller creado exitosamente')
    closeModal()
  } catch (error: any) {
...
const confirmDelete = (taller: any) => {
...
const confirmActivate = (taller: any) => {
...
onMounted(() => {
  console.log('Vista de talleres cargada')
})
</script>